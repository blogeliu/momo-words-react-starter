import React, { useState } from 'react'
import { useNavigate, Link } from 'react-router-dom'
import { updateDeck } from '../lib/sample'

function parseCards(text) {
  return text
    .split(/\r?\n/)
    .map(l => l.trim())
    .filter(Boolean)
    .map((line, idx) => {
      // Accept: "apple - 苹果", "water: 水", "book, 书", tabs, or 2+ spaces
      let [term, def] = line.split(/\s*[-,:]\s*|[\t]{1,}|\s{2,}/)
      if (!def) {
        const parts = line.split(/\s+/)
        term = parts[0]
        def = parts.slice(1).join(' ')
      }
      return {
        id: `c${idx + 1}`,
        term: (term || '').trim(),
        definition: (def || '').trim(),
        ease: 2.5,
        interval: 0,
        dueAt: Date.now()
      }
    })
    .filter(c => c.term && c.definition)
}

export default function CreateDeck() {
  const [name, setName] = useState('')
  const [rawCards, setRawCards] = useState('')
  const [err, setErr] = useState('')
  const navigate = useNavigate()

  function submit(e) {
    if (e) e.preventDefault()
    setErr('')

    const cards = parseCards(rawCards)
    if (!name.trim()) return setErr('Please enter a deck name.')
    if (cards.length === 0) return setErr('Please add at least one card.')

    const id = name
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, '-')
      .replace(/(^-|-$)/g, '') + '-' + Date.now()

    const deck = { id, name: name.trim(), createdAt: Date.now(), cards }
    updateDeck(deck)
    navigate('/decks')
  }

  return (
    <div>
      <h2>Create Deck</h2>
      <p>Enter a deck name and paste cards. One per line, like: <code>tennis - 网球</code></p>
      {err && <p style={{ color: '#ff6b6b' }}>{err}</p>}

      <form onSubmit={submit} style={{ display: 'grid', gap: 12 }}>
        <label>
          <div>Deck Name</div>
          <input
            style={{ width: '100%', padding: 10, borderRadius: 10 }}
            value={name}
            onChange={(e) => setName(e.target.value)}
            placeholder="HSK 1 Nouns (EN ↔ 中文)"
          />
        </label>

        <label>
          <div>Cards (one per line)</div>
          <textarea
            rows={10}
            style={{ width: '100%', padding: 12, borderRadius: 12 }}
            value={rawCards}
            onChange={(e) => setRawCards(e.target.value)}
            placeholder="tennis - 网球\nskiing - 滑雪\nprogrammer - 程序员"
          />
        </label>

        <div style={{ display: 'flex', gap: 10 }}>
          {/* Use type='button' + onClick to ensure it always fires */}
          <button className="btn" type="button" onClick={submit}>Create Deck</button>
          <Link className="btn" to="/decks">Cancel</Link>
        </div>
      </form>
    </div>
  )
}
